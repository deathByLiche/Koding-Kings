    ...
    public class dbManage
    {
       ...
       private  SQLiteDataReader sqlite_datareader;
        ...
        public Question[] getQuestions()
        {
            ...
            SetConnection();
            sqlite_conn.Open();
            for (int x = 1; x <= (count / 9);x++ )
            {
                Question q = new Question();
                sqlite_cmd = sqlite_conn.CreateCommand();
                sqlite_cmd.CommandText = "SELECT * FROM Questions WHERE rowid = ('" + x + "');";
                sqlite_datareader = sqlite_cmd.ExecuteReader();
                sqlite_datareader.Read();
                string temp;
                temp = sqlite_datareader["Question"].ToString();
                string []words = temp.Split(':');
                q.Category = words[0];
                q.Questions = words[1].Trim();
                q.Answer = sqlite_datareader["Answer1"].ToString();
                q.Dummy1 = sqlite_datareader["Answer2"].ToString();
                q.Dummy2 = sqlite_datareader["Answer3"].ToString();
                q.Dummy3 = sqlite_datareader["Answer4"].ToString();
                questions[x - 1] = q;
            }
            sqlite_conn.Close();
            return questions;
        ...
        private void CreateStartupDatabase()
        {
            ...
            ExecuteQuery("CREATE TABLE IF NOT EXISTS Questions (Question varchar(500),Key INT AUTO_INCREMENT, Answer1 varchar(100),a varchar(1),Answer2 varchar(100),b varchar(1),Answer3 varchar(100),c varchar(1),Answer4 varchar(100),d varchar(1));");
